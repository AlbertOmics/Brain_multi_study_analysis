...
```{r}
female_data <- joined_data[,joined_data$sra_attribute.Sex == 'male']
...
```{r}
female_data <- joined_data[,joined_data$sra_attribute.Sex == 'female']
tumor_projects <- unique(joined_data$study[joined_data$sra_attribute.tissue=='Tumor'])
tumor_data <- joined_data[,joined_data$study %in% tumor_projects]

female_data <- cbind(female_data,tumor_data)
f_vGene <- complete_analysis(female_data)
...
## Creemos una tabla con información de las muestras
## y con nombres de columnas más amigables
df <- as.data.frame(colData(female_data)[, c("sra_attribute.Sex", "prenatal","sra_attribute.tissue")])
colnames(df) <- c( "Sex","Prenatal", "Tissue")
...
lets save this to make studies
```{r}
save(raw_Data, file = "../processed_data/raw_studies.RData")
save(joined_data, file = "../processed_data/predict_sex_studies.RData")
